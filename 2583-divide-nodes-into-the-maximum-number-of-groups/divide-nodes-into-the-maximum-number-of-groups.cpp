class Solution {
private:
    void findConnectedComponent(int node, vector<vector<int>>& graph,
    ...
    int getMaxDepth(int start, unordered_set<int>& componentNodes,
...
public:
    int magnificentSets(int n, vector<vector<int>>& edges) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            int maxDepth = -1;
            for (int node : componentNodes) {
                maxDepth = max(maxDepth, getMaxDepth(node, componentNodes,
                                                     graph, visited));
            }
